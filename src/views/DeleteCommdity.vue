<template>
  ...
  >
    ...
    <el-table-column label="商品描述" prop="information"> </el-table-column>
    <el-table-column label="商品价格" prop="commodityPrice"> </el-table-column>
    <el-table-column label="拥有者" prop="byName"> </el-table-column>
...
export default {
  ...
  methods: {
    initData() {
      this.getRequest("/commodity-info/findAll").then((resp) => {
        if (resp) {
    ...
    handleDelete(index, row) {
      this.deleteRequest("/commodity-info/delete/" + row.id).then((resp) => {
        if (resp) {
